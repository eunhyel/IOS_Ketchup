...
open class PasswordContainerView: UIView {
    ...
    @IBOutlet weak var pass_1_imageview: UIImageView!
    @IBOutlet weak var pass_2_imageview: UIImageView!
    @IBOutlet weak var pass_3_imageview: UIImageView!
    @IBOutlet weak var pass_4_imageview: UIImageView!
    
    open var isEdit = false
    ...
    fileprivate let kDefaultWidth: CGFloat = 305
    fileprivate let kDefaultHeight: CGFloat = 510
    ...
    @IBAction func deleteInputString(_ sender: AnyObject) {
        if isEdit {
            if inputString.count <= 0 {
                getNavigationController().dismiss(animated: false, completion: nil)
            }
        }
