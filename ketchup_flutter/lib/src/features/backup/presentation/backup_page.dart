...
class _BackupPageState extends ConsumerState<BackupPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool cloudSync = settingsAsync.valueOrNull?.useCloudSync ?? false;
      ...
      children: <Widget>[
        ...
        if (Platform.isIOS) ...<Widget>[
          ...
          Positioned(
            ...
            height: 54 * scale,
            child: _ToggleBtn(
              text: cloudSync ? 'on' : 'off',
              enabled: !_busy,
              onTap: () {
                unawaited(_onCloudSyncToggle(current: cloudSync));
              },
            ),
          ),
        ],
  ...
  Future<void> _onCloudSyncToggle({required bool current}) async {
    if (_busy) {
      return;
    }
    ref.invalidate(icloudAccountAvailableProvider);
    ...
    if (next) {
      setState(() => _busy = true);
      try {
        await ref.read(appSettingsProvider.notifier).setUseCloudSync(true);
      } finally {
        if (mounted) {
  ...
  Future<void> _backup() async {
    ...
    try {
      ...
      await KetchupDriveService(api).backupIsar(isarBytes: bytes, appVersion: appVersion);
      _toast('백업 성공!');
    } catch (e) {
      _toast('백업 실패: $e');
    } finally {
